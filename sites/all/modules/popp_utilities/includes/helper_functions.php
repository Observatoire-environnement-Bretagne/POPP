...
function getInfosFromSeries()
{
    ...
    foreach ($nodes as $node) {
        ...
        fetchTaxonomyField($searchFields, $node, 'Axe thématique', t('Thématique'), 'popp_thematic_axes', 'field_popp_serie_thematic_axis');
...
function getThesaurusFromSeries()
{
    ...
    foreach ($nodes as $node) {
      ...
      //  if(in_array( $node->nid ,[51,1564,329])){
    ...
    foreach ($serie->field_popp_serie_photo_list[LANGUAGE_NONE] as $photoNid) {
        ...
        if (isset($photo->field_popp_photo_thesaurus[LANGUAGE_NONE])) {
            foreach ($photo->field_popp_photo_thesaurus[LANGUAGE_NONE] as $thesElt) {
                ...
                $putpar = putInParent($parents, $set, $set[$tid], $changes);
                $old_result = $result;
               
                
           
                $result = merge($result,$putpar, true);
                
                

                foreach ($old_result as $key1 => $value1){
                	                	 
                	foreach ($value1['child'] as $key2 => $value2){
                		foreach ($value2['child'] as $key3=>$value3)
                		
                		foreach ($value3['changes'] as $key4=>$value4){
                			
                		if(isset($value4) && count($value4)>0 ){
                			
                			if(isset($result['changes'][$key1][$key2][$key3][$key4])){
                				
                				
                				$liste =$result['changes'][$key1][$key2][$key3][$key4][0].",". $value4[0];
                				$liste1 = explode(",", $liste);
                				
                				$liste1 = array_unique($liste1);
                			
                				$result['changes'][$key1][$key2][$key3][$key4][0] = implode(",", $liste1);
                				
                			}else{
                				$result['changes'][$key1][$key2][$key3][$key4][0] = $value4[0];
                			}
